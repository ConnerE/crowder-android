...
const react_native_elements_2 = require("react-native-elements");
...
class Main extends React.Component {
    constructor(props) {
            ...
            crowdsRef.on("child_added", (snapshot) => {
                ...
                for (let key in members) {
                    ...
                    if (id == this.props.navigation.state.params.UUID) {
                        const newCrowd = { name: returnObj.name, key: snapshot.key, desc: returnObj.desc, dis: distance.toFixed(2).toString() + " kms away" };
        ...
        this.renderItem = (item) => {
            ...
            if (item.section.header !== "Explore Crowds") {
                return (<react_native_swipeout_1.default right={swipeBtns}>
                    <react_native_1.TouchableOpacity onPress={() => {
                    this.navigateToCrowd(item.item.key, item.item.name);
                }}>
                        <react_native_elements_2.ListItem roundAvatar title={item.item.name} subtitle={<react_native_1.View style={styles.subtitleView}>
                                    <react_native_1.Text style={styles.ratingText}>{item.item.dis}</react_native_1.Text>
                                </react_native_1.View>} underlayColor={"#FFFFFF"} badge={{ value: "0 messages", textStyle: { color: 'orange' }, containerStyle: { marginTop: 10 } }} containerStyle={{ backgroundColor: '#FFFFFF' }}/>
                ...
                return (<react_native_1.TouchableOpacity onPress={() => {
                    ...
                    <react_native_elements_2.ListItem roundAvatar title={item.item.name} subtitle={<react_native_1.View style={styles.subtitleView}>
                                <react_native_1.Text style={styles.ratingText}>{item.item.desc}</react_native_1.Text>
                            </react_native_1.View>} underlayColor={"#FFFFFF"} badge={{ value: item.item.dis, textStyle: { color: 'orange' }, containerStyle: { marginTop: 10 } }} containerStyle={{ backgroundColor: '#FFFFFF' }}/>
                </react_native_1.TouchableOpacity>);
            }
        };
    ...
    render() {
        return (<react_native_1.View style={styles.container}>
                <react_native_1.View style={{ marginBottom: 10, marginTop: -10 }}>
                    <react_native_elements_1.Button small backgroundColor="#33A6FF" onPress={this.editInfo} icon={{ name: 'envira', type: 'font-awesome' }} title='Edit My Information'/>
                </react_native_1.View>
...
Main.navigationOptions = ({ navigation }) => {
    return {
        ...
        headerRight: <react_native_elements_1.Icon name="add" color="#FFFFFF" size={35} onPress={() => {
